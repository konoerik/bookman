...
| B8 | — (parse) | ISBN on a copyright page deep in the front matter | Found within the first 10 pages; not beyond | ✅ 🟢 | Window widened 5 → 10 (2026-09-20) after a real bundle: every No Starch PDF puts the copyright page at index 5 (cover, blank, half-title, blank, title, copyright) and O'Reilly as far as 7, so 23 of 37 PDFs — and every No Starch one without a `/Title` — went ISBN-less and orphaned. `test_pdf::finds_isbn_on_a_copyright_page_after_four_pages_of_front_matter`, `ignores_isbn_beyond_scan_page_limit` (pinned as the limit) |
...
| E12 | IDENT-6 | Record accepted but has no cover (data gap) | Identified, `cover_path=None` — unless the file is an EPUB with an embedded cover (E15) | 🟢 | Live: Algorithmic Thinking 2nd ed., A Christmas Carol. Since ADR-28 this only leaves a book coverless when no format is an EPUB with a cover; then only another source or a user-supplied cover fixes it |
| E15 | IDENT-6 | No cover after identification (no record, record without one, or the download failed) and the file is an EPUB embedding one | The EPUB's own cover, as stored; a record's cover still wins, and a book with a cover keeps it | ✅ 🟢 | ADR-28 (2026-09-20), FIELD-NOTES FN-5: 21 of 37 real books were coverless while all 37 EPUBs embedded one. `formats.epub.extract_cover` reads EPUB 3 `properties="cover-image"`, EPUB 2 `<meta name="cover">`, or an item with id `cover` (the shapes seen: 27 / 37 / 37 of 37). `test_epub::extract_cover_*` (4); `test_library::uses_the_epubs_embedded_cover_when_there_is_no_record`, `…when_the_record_has_none`, `…when_the_download_fails`, `prefers_the_records_cover_to_the_embedded_one`, `follow_up_epub_fills_a_missing_cover_but_keeps_an_existing_one` |
...
| F16 | GROUP-4 | Order independence: `{epub without ISBN, pdf with ISBN}` imported in either order | Same end state | ✅ ⚠️ | `test_library::import_order_does_not_change_the_end_state` — title, author, ISBN, `identified`, `grouped`, `needs_review` and format set all equal. **Known exception** since ADR-26: `{epub asserting the ebook ISBN, pdf scraping print + ebook}` joins EPUB-first but not PDF-first (the book remembers one ISBN, the print one, and the EPUB's never meets it) — spec OQ4, FIELD-NOTES FN-4 |
| F17 | GROUP-1 | File carries several ISBNs (print + ebook + prior editions scraped from a copyright page); an existing book holds one of them that is *not* the one identification settled on | Joins on the shared one, `grouped=isbn` | ✅ 🟢 | ADR-26 (2026-09-20). `Identification.isbns` carries every ISBN the file still claims (rejected ones excluded); `_find_book` tries each. `test_resolve::carries_every_isbn_the_file_still_claims`, `carries_isbns_the_source_does_not_know`; `test_library::joins_on_any_isbn_the_file_carries`. Live: 4 No Starch pairs (FN-4) |
| F18 | GROUP-4 | ISBN join, and the follow-up was identified through a *different* ISBN than the one it joined on (a title-less PDF citing the previous edition's number, which is the only one the source knows) | Grouped, but the book keeps its title, cover and `identified` | ✅ 🟢 | ADR-27 (2026-09-20). Before it the follow-up's "stronger" identification renamed *Rust 3rd ed.* to the 2nd-edition record and would have fetched its cover. A record reached through the joined ISBN still upgrades (F7). `test_library::on_an_isbn_join_declines_a_record_reached_through_another_isbn`, `…adopts_a_record_reached_through_the_joined_isbn` |
...
| I3 | Cover image | Browse view | ✅ | `Book.cover_path`; fetched from the record, else the EPUB's embedded one (E15, ADR-28), never user-supplied (see K4). Always named `cover.png` but the bytes are as received — JPEG more often than not — so a frontend should sniff the format, not trust the suffix |
...
| K4 | Supply / replace / remove a cover (file or URL) | The route to a cover for E12 books with no EPUB, and to *replace* a wrong one | ❌ | Backlog; needs I3 to accept a user-supplied image. Less pressing since E15 |
...
- **E12** (no cover on Open Library) — E15 covers it when an EPUB is present; otherwise only K4 or N8.
